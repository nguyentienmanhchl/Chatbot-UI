        ...
        if (val.length < 1) {
            ...
            for (i = 0; i < val.length; i++) {
                ...
                if (val[i].hasOwnProperty("text")) {
                    ...
                    var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${val[i].text.split("~")[0]}</span></div>`;
                    $(BotResponse).appendTo('.chat-area').hide().fadeIn(1000);
                    if(val[i].text.split("~").length>1){
                        var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${val[i].text.split("~")[1]}</span></div>`;
                        $(BotResponse).appendTo('.chat-area').hide().fadeIn(1000);
                    }
